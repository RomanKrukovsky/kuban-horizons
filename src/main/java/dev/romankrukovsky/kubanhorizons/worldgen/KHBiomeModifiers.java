...
public final class KHBiomeModifiers {
    ...
    public static final ResourceKey<BiomeModifier> ADD_WILD_FRUIT_TREES =
            createKey("add_wild_fruit_trees");
    ...
    public static void bootstrap(BootstrapContext<BiomeModifier> context) {
        ...
        // Одичавшие плодовые деревья — заброшенные кубанские сады.
        //
        // Все четыре вида идут одним модификатором и одним набором биомов:
        // по отдельности они разошлись бы по карте так, что для полного
        // сада пришлось бы обойти полмира. Здесь же встреченная роща даёт
        // шанс сразу на несколько культур — это и есть «нашёл старый сад».
        //
        ...
        context.register(ADD_WILD_FRUIT_TREES, new BiomeModifiers.AddFeaturesBiomeModifier(
                ...
                HolderSet.direct(
                        features.getOrThrow(KHPlacedFeatures.WILD_PEACH_TREE_PLACED),
                        features.getOrThrow(KHPlacedFeatures.WILD_APRICOT_TREE_PLACED),
                        features.getOrThrow(KHPlacedFeatures.WILD_PLUM_TREE_PLACED),
                        features.getOrThrow(KHPlacedFeatures.WILD_WALNUT_TREE_PLACED)),
                GenerationStep.Decoration.VEGETAL_DECORATION));
