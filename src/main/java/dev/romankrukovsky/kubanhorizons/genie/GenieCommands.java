...
public final class GenieCommands {
    ...
    private static int converse(ServerPlayer player, String message) {
        KubanGenie genie = GenieConversationService.nearestOwned(player);
        if (genie == null) {
            player.sendSystemMessage(Component.translatable("message.kubanhorizons.genie.ai.no_companion"));
            return 0;
        }
        GenieConversationService.Response response = GenieConversationService.submitWish(
                player, genie.getId(), message);
        player.sendSystemMessage(response.message());
        return response.emotionLevel() > 0 || response.confirmationRequired() ? 1 : 0;
    }
}
