...
/** Reversible policy runtime для глобальных правил мира. */
public final class PolicyService {
    ...
    private volatile boolean instantSmeltEnabled;
    ...
    public PolicyPreview previewInstantSmelt(UUID actor, boolean target) {
        return new PolicyPreview(UUID.randomUUID(), actor, INSTANT_SMELT,
                Boolean.toString(instantSmeltEnabled), Boolean.toString(target),
                Instant.now().plus(PREVIEW_TTL));
    }
    ...
    public TransactionReport execute(UUID actor, MinecraftServer server, ConfirmedPolicy confirmed)
        ...
        retireCommitted(server, confirmed.preview().ruleId());
        UUID id = UUID.randomUUID();
        ...
        return new TransactionReport(id, TransactionOutcome.COMPLETED, 0,
                prepared.ruleId() + " policy committed");
    }
    ...
    public TransactionReport undo(UUID actor, MinecraftServer server, UUID transactionId)
        ...
        return new TransactionReport(transactionId, TransactionOutcome.COMPLETED, 0,
                manifest.ruleId() + " policy reverted");
    }

    public void recover(MinecraftServer server) throws IOException {
        instantSmeltEnabled = false;
        List<PolicyManifest> manifests = store.list().stream()
                .sorted(Comparator.comparing(PolicyManifest::createdAt))
                .toList();
        for (PolicyManifest manifest : manifests) {
            if (manifest.state() == PolicyManifest.PolicyState.PREPARED) {
                apply(server, manifest.ruleId(), manifest.beforeValue());
                store.save(new PolicyManifest(manifest.transactionId(), manifest.actorId(), manifest.ruleId(),
                        manifest.beforeValue(), manifest.targetValue(), manifest.createdAt(),
                        PolicyManifest.PolicyState.ROLLED_BACK));
            } else if (manifest.state() == PolicyManifest.PolicyState.COMMITTED) {
                apply(server, manifest.ruleId(), manifest.targetValue());
            }
        }
    }
    ...
    private void retireCommitted(MinecraftServer server, String ruleId) throws IOException {
        for (PolicyManifest manifest : store.list()) {
            if (manifest.state() != PolicyManifest.PolicyState.COMMITTED
                    || !manifest.ruleId().equals(ruleId)) {
                continue;
            }
            apply(server, manifest.ruleId(), manifest.beforeValue());
            store.save(new PolicyManifest(manifest.transactionId(), manifest.actorId(), manifest.ruleId(),
    ...
    private String read(MinecraftServer server, String ruleId) {
    ...
    private void apply(MinecraftServer server, String ruleId, String value) {
        switch (ruleId) {
